  ...
  const handleMarkWatched = async (movieId: string) => {
    ...
    try {
      const { error } = await supabase
        ...
        .eq('id', movieId)
        .eq('client_email', userEmail); // Strict user permission isolation validation
            ...
            {displayList.map((movie) => (
              ...
              >
                ...
                <div className="relative z-20 space-y-2">
                  <h4 className="font-black text-xs text-gray-900 line-clamp-2 leading-tight">{movie.title}</h4>
                  {movie.release_year && (
                  ...
                  {subTab === 'watchlist' && (
                    <button 
                      type="button"
                      disabled={isMutating === movie.id}
                      onClick={() => handleMarkWatched(movie.id)}
                      className="w-full py-2 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-xl text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-1 shadow-sm hover:opacity-95 focus:outline-none transition-opacity"
                    >
                      {isMutating === movie.id ? (
